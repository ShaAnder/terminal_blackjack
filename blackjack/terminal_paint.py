...
def pos(text, xcoords, ycoords, color):
    """
...
def intro(display_cards):
    ...
    pos(term.center("A game of wits and chance, Press any key to continue"), 0, 27, term.black_on_yellow)
...
def instructions():
    ...
    with term.location(0, 7):
        ...
        pos(term.center("Press any key begin the game"), 0, 27, term.black_on_yellow)
        with term.cbreak(), term.hidden_cursor():
            term.inkey()
...
def get_user_input(terminal_row, message):
    """
    Takes the value entered, passes it into the validatior, checks to see if it
    is either "Hit" or "Stay" then returns that string to tell the program
    what to do next.
    Args:
...
def win(condition_message):
    ...
    with term.location(0, 13):
        for line in layout.win_heading.split("\n"):
            print(term.yellow + term.center(line))
    pos(term.center(condition_message), 0, 23, term.black_on_yellow)
...
def calculating(message):
    ...
    """
    pos(term.center(message), 0, 34, term.white_on_blue)
...
def cont():
    ...
    """
    pos(term.center("Press any key to continue the game..."), 0, 34, term.black_on_yellow)
    with term.cbreak(), term.hidden_cursor():
        term.inkey()
